...
@property(nonatomic,strong) UIView * clipView1;
...
- (instancetype)initWithFrame:(CGRect)frame andTitleArray:(NSArray *)titleArr
{
    if (self = [super initWithFrame:frame])
    {
        ...
        {
            [self addSubview:[self createLabel:titleArr[i] andFrame:CGRectMake(i*_width, 0, _width, self.frame.size.height) andColor:[UIColor systemBlueColor]]];
        }
        
        //滑块
        _clipView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        ...
        _clipView1 = [[UIView alloc]initWithFrame:CGRectMake(self.frame.size.width, 0, self.frame.size.width, self.frame.size.height)];
        _clipView1.backgroundColor = [UIColor systemPinkColor];
        //设置不显示超出的部分
        _clipView1.clipsToBounds = YES;
        _clipView1.userInteractionEnabled = YES;
        [self addSubview:_clipView1];
        ...
        for (int i = 0; i < titleArr.count ; i++)
        {
            [_showView addSubview:[self createLabel:titleArr[i] andFrame:CGRectMake(i*_width, 0, _width, self.frame.size.height) andColor:[UIColor yellowColor]]];
        }
...
-(void)panView:(UIPanGestureRecognizer *)sender
{
    ...
    CGPoint clipCenter1 = _clipView1.center;
